...
function StateButton(stateMachineJSON, attribute, text, onClick) {
...
// This is a button which enables/disables an action based on a given attribute.
function StateLabel(stateMachineJSON, attribute, texts) {
    this.attribute = attribute;
    this.onClick = onClick;
    this.button = document.createElement("input");
    this.button.type = "button";
...
StateLabel.prototype.update = function(stateMachineJSON) {
    this.enabled = stateMachineJSON[this.attribute] ? true : false;
    this.button.disabled = !this.enabled;
    if (this.enabled) {
        this.button.classList.add("enabled");
...
StateLabel.prototype.getHTMLElement = function() {
    return this.button;
}
